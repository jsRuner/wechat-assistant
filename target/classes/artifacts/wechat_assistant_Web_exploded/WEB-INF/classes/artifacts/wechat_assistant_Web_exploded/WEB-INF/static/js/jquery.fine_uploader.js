...
(function(global) {
    ...
    (function() {
                ...
                if (data) {
                    ...
                    qq.each(data, function(idx, fileContainer) {
                        if (qq.isFileOrInput(fileContainer)) {
                            self._options.request.params.name = fileContainer.name;
                            self._options.request.params.filename = fileContainer.name;                            
        ...
        qq.basePrivateApi = {
            ...
            _getKey: function(){
                var key = this._options.request.params.key + this._options.request.params.folder + this._options.request.params.folder_time + this._calculate_object_name(this._options.request.params.filename);
                return key;
            },
            ...
            _calculate_object_name: function(filename){
                var suffix = this._getSuffix(filename);
                var g_object_name = this._randomString(10) + suffix;
                return g_object_name;
            },
            //获取参数。
            //向阿里云传递参数。http://dms-boluo361.oss-cn-hangzhou.aliyuncs.com/
            _getPolicy: function(input){
                var _this = this;
                var _params = null;
                $.ajax({
                    url: '/api/aliyun_oss/get_policy',
                    async: false,
                    dataType: 'json',
                    type: 'get',
                    success: function(res){
                        //_params = res;
                        _this._options.request.params.OSSAccessKeyId = res.accessid; //用户id
                        _this._options.request.params.callback = res.callback; //回调地址和回调方法
                        _this._options.request.params.key = res.dir; //文件保存目录
                        //_this._options.request.endpoint = res.host;
                        _this._options.request.params.policy = res.policy; //阿里云策略
                        _this._options.request.params.signature = res.signature; //签名。
                    }
                });
            },
            ...
            _onValidateCallbackSuccess: function(items, index, params, endpoint) {
                ...
                this._validateFileOrBlobData(items[index], validationDescriptor).then(function() {
                    self._getPolicy();
                    self._options.request.params.key = self._getKey();
